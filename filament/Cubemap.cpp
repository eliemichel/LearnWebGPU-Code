...
Cubemap::Address Cubemap::getAddressFor(const vec3& r) {
...
void Cubemap::makeSeamless() {
    ...
    const ssize_t bpr = (ssize_t)getImageForFace(Face::NX).getBytesPerRow();
    const ssize_t bpp = (ssize_t)getImageForFace(Face::NX).getBytesPerPixel();
    ...
    auto corners = [ & ](Face face) {
        ...
        *getTexel(image,  -1,  -1) = (*getTexel(image, 0, 0) + *getTexel(image,  -1,  0) + *getTexel(image, 0,    -1)) / 3.0f;
        *getTexel(image, L+1,  -1) = (*getTexel(image, L, 0) + *getTexel(image,   L, -1) + *getTexel(image, L+1,   0)) / 3.0f;
        *getTexel(image,  -1, L+1) = (*getTexel(image, 0, L) + *getTexel(image,  -1,  L) + *getTexel(image, 0,   L+1)) / 3.0f;
        *getTexel(image, L+1, L+1) = (*getTexel(image, L, L) + *getTexel(image, L+1,  L) + *getTexel(image, L+1,   L)) / 3.0f;
    };
